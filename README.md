# РезюмИТ Найм — MAX-бот первичного отбора

Актуальный продукт хакатона: **бот для работодателя и кандидата внутри MAX**. Создание вакансии, подтверждение требований, приём отклика, до трёх уточнений, просмотр фактов и ручное приглашение — без обязательного сайта.
...
- Для production используется HTTPS webhook, не polling.
- `.env.hiring`, токены, база откликов и SSH-ключи не включаются в Git.
- Тестовая версия: используйте вымышленные резюме. Подключение реальных работодателей требует настройки оператора данных и проверки в MAX.

## Исходный проект Вадима (справка)

Ниже сохранено описание исходной системы. Его LLM, LaTeX и старый scoring **не используются** новым чат-сценарием.
